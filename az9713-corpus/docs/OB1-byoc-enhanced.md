---
repo: OB1-byoc-enhanced
description: Fork of NateBJones-Projects/OB1 with v1.4 BYOC enhancements: 3 new context layers, scope-tag namespaces, pre-export trade-secret audit, differential re-interview. Closes gaps named in Nate Jones' Apr 17 2026 substack post.
language: TypeScript
stars: 0
forks: 0
created: 2026-04-17
...
> **This is a fork of [NateBJones-Projects/OB1](https://github.com/NateBJones-Projects/OB1).** The upstream README begins below the divider. Everything between this banner and the divider is fork-specific.
>
> **What this fork adds:** an enhanced `recipes/bring-your-own-context/` workflow (v1.1 → v1.4) that closes specific gaps Nate Jones identified in [*The Six-Month AI Context You Lose Every Time You Switch Tools, Jobs, Or Employers*](https://natesnewsletter.substack.com/p/the-ai-capital-youve-been-building) (Apr 17 2026). The enhancements are additive, idempotent, and FSL-1.1-MIT compatible.
>
> The full design and implementation are documented in [`docs/byoc-comprehensive-guide.md`](docs/byoc-comprehensive-guide.md).

### Pain points → Layer 1 enhancements

| Nate's pain point (source) | Our enhancement | Where it lives |
|----------------------------|-----------------|----------------|
| Domain encoding loss — vocabulary, products, regulations, acronyms accumulated drip-by-drip | New `domain_encoding` WOM layer with vocabulary, products, competitors, acronyms, strategy patterns | [`recipes/work-operating-model-activation/`](recipes/work-operating-model-activation/) (PR 1, v1.1) |
| Behavioral relationship loss — "the nose you can't see"; unstated prefs from micro-corrections | New `behavioral_relationship` WOM layer with `challenge_vs_execute`, `technical_depth_default`, `preamble_tolerance`, `correction_examples[]` | [`recipes/work-operating-model-activation/`](recipes/work-operating-model-activation/) (PR 1, v1.1). Acknowledged limitation: deepest calibration cannot fully port. |
| Demonstrated capability invisible — "buried in 800 chats"; no portfolio format | New `artifacts` WOM layer + `ARTIFACTS.md` export with `artifact_type`, `tools_used[]`, `prompt_chain_summary`, `outcome`, `evidence_uri`, `shareable` | [`recipes/work-operating-model-activation/`](recipes/work-operating-model-activation/) (PR 1, v1.1) |
| Personal vs professional separation — "no one is addressing it well today" | `scope_tags` namespace vocabulary (`personal` / `professional` / `sensitive` / `employer:<id>`) on every thought + every operating-model entry | [`server/index.ts`](server/index.ts) + [`recipes/work-operating-model-activation/`](recipes/work-operating-model-activation/) (PR 2, v1.2) |
| Job change boundary — separating yours from employer's | Two-cabinet pattern: `partition_by_scope: true` on export emits one bundle per namespace; `employer:<id>` namespace stays behind on exit | [`recipes/work-operating-model-activation/`](recipes/work-operating-model-activation/) (PR 2, v1.2) |
| Trade-secret stripping — "specific Q3 pricing → general B2B pricing heuristic" | New `context-audit` primitive with 10-category rubric, severity ladder, three modes (`off` / `advisory` / `strict`), heuristic + LLM classifier passes; emits `audit_report.json` artifact | [`primitives/context-audit/`](primitives/context-audit/) + [`recipes/work-operating-model-activation/`](recipes/work-operating-model-activation/) (PR 3, v1.3) |
| Profile must evolve as career grows — "should grow with us" | New `start_diff_session` MCP tool keyed off `profile_version`: classifies layers as stale or fresh, carries fresh layers forward, stages only stale ones for re-interview. `since_version` query parameter shows what changed between versions. | [`recipes/work-operating-model-activation/`](recipes/work-operating-model-activation/) + [`skills/work-operating-model/`](skills/work-operating-model/) (PR 4, v1.4) |
| Hiring market portfolio gap — "leveraged AI tools" = unfalsifiable noise | `ARTIFACTS.md` rendered from the artifacts layer with `shareable: true` flag for public publication; `evidence_uri` per entry | [`recipes/work-operating-model-activation/`](recipes/work-operating-model-activation/) (PR 1 + PR 2, v1.1–v1.2) |
| Platform switch boundary — QuitGPT-style migration | `scope_filter` on export produces a clean cross-platform slice; user-controlled MCP store carries the bundle | [`recipes/work-operating-model-activation/`](recipes/work-operating-model-activation/) (PR 2, v1.2) — extends v1.0 |
| Behavioral calibration gap (Nate's own acknowledgment) | Honest non-closure documented; `correction_examples[]` field is the best Layer 1 can offer | [`docs/byoc-comprehensive-guide.md`](docs/byoc-comprehensive-guide.md) §FAQ |

**Pain points NOT addressed by this fork:**
- *Diffuse pain shape that kills memory-startup adoption* — distribution problem, not a feature problem. Out of scope.
- *Platform incentive misalignment* — this fork routes around platforms via user-owned infrastructure; it does not change platform behavior.
- *IT departments blocking external MCP* — the two-cabinet pattern is the architectural argument; cannot force corporate policy.
- *Cross-account merge inside a single platform (e.g. Anthropic personal ↔ enterprise)* — platform-side policy.

**Compatibility:** every change is additive. The `schema.sql` carries an idempotent `DO $$ ... $$` migration block that detects v1.0 constraints and extends them in place. Re-running on a v1.0 database is safe; existing rows continue to validate. Optional `OPENROUTER_API_KEY` env var on the WOM activation edge function enables the audit's LLM classifier pass; without it the audit falls back to deterministic regex heuristics. No breaking changes for v1.0 consumers.
