---
repo: ECC-tutorial
description: Fork of affaan-m/ECC enhanced with comprehensive documentation and an interactive rate-limiting demo showcasing ECC's 3 layers
language: JavaScript
stars: 1
forks: 0
created: 2026-05-28
...
> **This is a community fork of [affaan-m/ECC](https://github.com/affaan-m/ECC) enhanced with:**
>
> - **[Comprehensive documentation](docs/)** — 21 docs covering concepts, guides, reference, and troubleshooting. Built for zero-to-productive in one sitting.
> - **[Interactive demo](demo/)** — A complete rate-limiting feature implementation that shows all 3 ECC layers firing in sequence, with 5 deliberate bugs that each layer catches at exactly the right moment.
>
> If you want the original upstream repo, go to [affaan-m/ECC](https://github.com/affaan-m/ECC). If you want to understand how ECC works from scratch, start here.

---

## The Demo

`demo/` walks through adding rate limiting to an Express API with 5 deliberate bugs. Each bug is caught by a different ECC layer, at a different stage:

| Bug | Layer | When caught |
|-----|-------|-------------|
| Hardcoded `REDIS_URL = "redis://localhost:6379"` | **Layer 3: Hook** | Before `npm start` runs — `pre:bash:dispatcher` blocks the command |
| `const ip: string = req.headers['x-forwarded-for']` (TS2322) | **Layer 3: Hook** | Seconds after file is saved — `post:edit:typecheck` fires |
| No `Retry-After` header on 429 response | **Layer 2: Agent** | `/code-review` — `code-reviewer` flags RFC 6585 violation |
| IP key from `X-Forwarded-For` alone — spoofable | **Layer 2: Agent** | `/security-scan` — `security-reviewer` flags OWASP A05 |
| No adversarial test for IP spoofing | **Layer 1: Skill** | Before implementation — `tdd-workflow` skill requires the test |

**→ [Read the full receipt: demo/SHOWCASE.md](demo/SHOWCASE.md)**
...
cd demo/app
npm install
npm test          # 10 tests pass, 100% line coverage on middleware
npx tsc --noEmit  # 1 deliberate TS2322 error in rateLimiter.ts, 0 in rateLimiter.fixed.ts
```
