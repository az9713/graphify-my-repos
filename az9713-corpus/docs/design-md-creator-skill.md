---
repo: design-md-creator-skill
description: Fork of awesome-design-md + design-md-creator Claude Code skill. Generates DESIGN.md files for any brand via live WebFetch research, demonstrated with CMU → AIU landing page.
language: HTML
stars: 2
forks: 1
created: 2026-04-11
...
> **Fork of [VoltAgent/awesome-design-md](https://github.com/VoltAgent/awesome-design-md)** — extended with a `design-md-creator` Claude Code skill that generates new DESIGN.md files for any brand, demonstrated end-to-end with a fictional university landing page.

---

## What this fork adds: closing the loop

The original `awesome-design-md` repo provides 58+ hand-crafted DESIGN.md files for real-world brands.
This fork extends the concept in three steps that close the loop between **research → generation → application**:

### Step 1 — Reverse-engineer the spec
We read all 58 DESIGN.md files and extracted a formal specification ([`DESIGN_MD_SPEC.md`](DESIGN_MD_SPEC.md)) documenting the exact 9-section format, table schemas, color entry conventions, typography rules, quality tiers, and 13 authoring principles.

### Step 2 — Build a `design-md-creator` Claude Code skill
Using the spec and a full-tier example (Stripe), we authored a Claude Code skill installed in `.claude/skills/design-md-creator/`. The skill:
- Reads the spec and reference example
- Uses `WebFetch` to **live-research any brand's visual identity** from their public website
- Generates a complete, spec-compliant DESIGN.md with real hex codes, real font names, and real spacing values — not placeholders

Invoked as: `/design-md-creator <brand or URL>`

### Step 3 — Apply the skill, build a page
We invoked the skill on **Carnegie Mellon University** (`https://www.cmu.edu/`), fetching their live brand guidelines to extract Carnegie Red `#c41230`, Gold Thread `#fdb515`, Open Sans, and Source Serif 4. The result: [`design-md/cmu/DESIGN.md`](design-md/cmu/DESIGN.md).

We then fed that DESIGN.md to Claude and asked it to build a landing page for a **fictitious university — AIU (Agentic Intelligence University)** — "University in the Age of AI Agents":

![AIU landing page](design-md/cmu/AIU.jpg)

The full page is at [`design-md/cmu/AIU.html`](design-md/cmu/AIU.html). It implements every token from the DESIGN.md: color variables, font stacks, shadow formulas, border-radius philosophy, typography scale, responsive breakpoints, and component patterns — all faithfully derived from a live brand website via a single skill invocation.

**The loop:** 58 examples → spec → skill → new DESIGN.md from any URL → production-ready UI component.
