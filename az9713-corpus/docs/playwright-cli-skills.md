---
repo: playwright-cli-skills
description: Playwright UI Testing Suite — 16 skills, ~517 test cases for comprehensive web app quality testing
language: JavaScript
stars: 0
forks: 0
created: 2026-03-09
...
Comprehensive browser-based UI testing toolkit for [Claude Code](https://claude.ai/claude-code). 17 skills, ~482+ automated test cases, zero configuration. Point any skill at a URL and get a structured report with evidence.
...
> **Auto-install**: Every skill checks for prerequisites before running and will auto-install missing dependencies (Playwright, browser binaries, axe-core). You can skip this with `--skip-prereqs`.
...
Four new features inspired by the workflow demonstrated in [Claude Code + Playwright: Test Any App in 10 Mins for $0](https://www.youtube.com/watch?v=O4_0IaIZKFI&t=15s) by Scheffel:

| Feature | What It Does | Why It Matters |
|---------|-------------|----------------|
| **Fix Loop** | When tests fail, automatically retries up to 3 times — categorizing each failure as a test bug or app bug, fixing the right thing, and re-running | Previously, all 16 skills ran tests and reported results but never attempted self-repair. The fix loop is the biggest workflow improvement — it reduces false negatives and turns a single test run into an iterative debugging session |
| **Prerequisites Check** | Before any skill runs, verifies Node.js, Playwright, browser binaries, and target URL reachability — auto-installs what's missing | Without this, running a skill on a fresh machine produced confusing errors. Now it just works out of the box |
| **Headed Mode** | Pass `--headed` to see browser windows during testing (default: headless) | Essential for debugging — watch the browser interact with your site in real-time instead of reading test reports after the fact |
| **Test Generation** | New `/test-generate` skill reads your codebase and generates project-specific `.spec.ts` files that run via `npx playwright test` at zero token cost | The existing skills run generic tests against any URL. This skill produces tailored tests for your actual routes, forms, and flows — and they persist as part of your project |

### Flags Reference

| Flag | Effect | Example |
|------|--------|---------|
| `--headed` | Show browser windows during testing | `/test-a11y https://myapp.com --headed` |
| `--skip-prereqs` | Skip environment prerequisite checks | `/test-forms https://myapp.com --skip-prereqs` |
| `--baseline` | Capture regression baseline (test-regression only) | `/test-regression https://myapp.com --baseline` |

### Future Features (Planned)

- **`playwright.config.ts` Generation** — Standalone utility to generate a `playwright.config.ts` for any project, auto-detecting framework, `webServer` command, browser projects, and reporters. Currently part of `/test-generate` Phase 4; may become a separate `/test-config` skill.
- **Test File Persistence** — `--persist` flag to save generated/passing tests into your project's `tests/e2e/` directory with a README explaining how to re-run them independently via `npx playwright test`. Turns one-time test runs into permanent regression suites.
...
| Test Generate | `/test-generate` | — | Analyze codebase, generate project-specific `.spec.ts` files |
...
|-------|---------|-------------|
| Full Suite | `/test-all <url>` | Runs all 16 skills in 5 phased waves (~482+ tests) |
...
/test-forms https://your-app.com --headed

# Generate project-specific tests
/test-generate http://localhost:3000
```
...
> "Generate Playwright tests for my app based on the codebase"
>
> "Create e2e tests that cover all the routes in my Next.js project"
>
> "Write tests for my forms and user flows automatically"
...
- New features: fix loop, prerequisites, headed mode, test generation
- Recipes and workflows (pre-launch, post-deploy, landing page optimization, etc.)
- Troubleshooting guide

See [V1.1-FEATURE-TESTING.md](V1.1-FEATURE-TESTING.md) for v1.1 feature verification results — what was tested, what passed, what wasn't tested, and recommended follow-up tests.
...
.claude/skills/playwright-ui-testing/
  ...
  references/
    ...
    prerequisites.md                # Environment verification, auto-install
    fix-loop.md                     # Automated test repair process
  ...
  test-generate/SKILL.md            # Project-specific test generation
...
- [Claude Code + Playwright = INSANE Browser Automations](https://www.youtube.com/watch?v=I9kO6-yPkfM&t=40s) by [Chase AI](https://www.youtube.com/@_chase_ai) — the original inspiration for building a Playwright-powered testing suite for Claude Code.
- [Claude Code + Playwright: Test Any App in 10 Mins for $0](https://www.youtube.com/watch?v=O4_0IaIZKFI&t=15s) by Scheffel — inspired the v1.1 features: automated fix loop, prerequisites checking, headed/headless toggling, and per-project test generation. Scheffel's video demonstrated a practical, opinionated workflow for generating project-specific tests from codebase analysis that directly shaped the `/test-generate` skill and the fix loop process.
