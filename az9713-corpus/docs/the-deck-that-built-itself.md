---
repo: the-deck-that-built-itself
description: A recursive, autonomous HTML slide deck: Claude Code installed an AI skill and invoked it to generate slides about that very process.
language: JavaScript
stars: 0
forks: 0
created: 2026-06-24
...
| [`.claude/skills/frontend-slides/`](./.claude/skills/frontend-slides/) | Zara Zhang's skill, vendored at **project scope** so anyone cloning this repo can regenerate or extend the deck. |
| [`.claude/skills/ouroboros/`](./.claude/skills/ouroboros/) | A new skill **born from this project** — it packages the whole loop (read a source → install a skill → hot-load → invoke it to document its own creation → publish) so the workflow can be re-run on anything. |
...
And true to the deck's own thesis — *"you don't start with a skill, you end with one"* — this project ended by producing a skill: [`ouroboros`](./.claude/skills/ouroboros/), which packages this entire self-documenting build loop so it can be run again on any source.
