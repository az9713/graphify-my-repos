---
repo: understand-anything-tutorial
description: Tutorial fork of Lum1104/Understand-Anything — enhanced with comprehensive docs and 6 new agent-integration features
language: TypeScript
stars: 0
forks: 0
created: 2026-05-20
...
# understand-anything-tutorial
> **This is a tutorial fork of [Lum1104/Understand-Anything](https://github.com/Lum1104/Understand-Anything).**
> Enhanced with comprehensive onboarding documentation and 6 new agent-integration features built on top of the original codebase — no original files were modified.
> For a video walkthrough of what Understand Anything does, watch **[This AI Tool Maps Any Codebase Before You Touch It](https://www.youtube.com/watch?v=VmIUXVlt7_I&t=22s)** on YouTube.
...
| Doc | What it covers |
|---|---|
| [How the Graph Is Built](docs/how-the-graph-is-built.md) | Every phase of the 7-agent pipeline — what each agent does, what's deterministic vs LLM, how edges are produced |
| [Beyond the Graph](docs/beyond-the-graph.md) | Dashboard, `/understand-chat`, `/understand-diff`, `/understand-explain`, auto-update, multi-platform, team sharing |
| [Agent Usage](docs/agent-usage.md) | How AI agents read and use the knowledge graph — grep patterns, jq queries, skill invocations, concrete examples |
| [Agent Integration Gaps](docs/agent-integration-gaps.md) | Six confirmed gaps with evidence from source code, each with an implementation plan |
| [Graph Schema Reference](docs/graph-schema.md) | Every field in `knowledge-graph.json` — 21 node types, 35 edge types, layer and tour schemas, jq query examples |

### New features (`understand-anything-plugin/`)

All six are purely additive — zero existing files modified.

| Feature | Gap it closes | What it does |
|---|---|---|
| [MCP server](understand-anything-plugin/packages/mcp-server/) | No structured agent tool access | Exposes 8 typed MCP tools (`search_nodes`, `get_node`, `get_neighbors`, `get_subgraph`, `get_layers`, `get_tour`, `check_staleness`, `search_domain`) via stdio transport |
| [`/understand-context` skill](understand-anything-plugin/skills/understand-context/SKILL.md) | Graph summary never reaches agents | Writes a structured architecture summary into `CLAUDE.md`, `AGENTS.md`, or `.github/copilot-instructions.md` — every future agent session starts with codebase context |
| [`/ua-status` skill](understand-anything-plugin/skills/ua-status/SKILL.md) | Stale graph invisible to agents | Compares `meta.json` commit hash against git HEAD; reports changed files and recommends refresh |
| [`/ua-chat` skill](understand-anything-plugin/skills/ua-chat/SKILL.md) | Domain graph unreachable via skills | Drop-in replacement for `/understand-chat` that also searches `domain-graph.json` — returns ordered business-flow steps with file+line pointers |
| [`ua-check-staleness.mjs`](understand-anything-plugin/scripts/ua-check-staleness.mjs) | Auto-update is Claude Code-only | Standalone git post-commit hook script for Codex, Cursor, Gemini CLI, and any other platform |
| [`ua-generate-embeddings.mjs`](understand-anything-plugin/scripts/ua-generate-embeddings.mjs) + [`/ua-embeddings`](understand-anything-plugin/skills/ua-embeddings/SKILL.md) | `SemanticSearchEngine` unwired | Generates vector embeddings via any OpenAI-compatible API; MCP server uses them automatically for semantic search |
